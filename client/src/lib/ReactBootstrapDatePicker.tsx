...
const Calendar = createReactClass({
  ...
  getInitialState() {
    const state = {seed: (new Date()).valueOf()};
    return state;
  },
      ...
      for (let j = 0; j <= 6; j++) {
        if (day <= monthLength && (i > 0 || j >= startingDay)) {
          ...
          if (beforeMinDate || afterMinDate) {
            week.push(
              <td
                key={`day${day}_${this.props.fieldKey}_1`}
            ...
            week.push(
              <td
                key={`day${day}_${this.props.fieldKey}_2`}
          ...
          week.push(<td key={`empty${j}_${this.props.fieldKey}`} />);
        }
      }
...
export default createReactClass({
  ...
  makeDateValues(isoStringRaw) {
    let displayDate;
    let isoString = typeof isoStringRaw === "string"? isoStringRaw : (
        isoStringRaw && isoStringRaw['value'] ? isoStringRaw['value'] : undefined
      );
    const selectedDate = isoString ? new Date(`${isoString.slice(0,10)}T12:00:00.000Z`) : null;
    const minDate = this.props.minDate && isoString ? new Date(`${isoString.slice(0,10)}T12:00:00.000Z`) : null;
    const maxDate = this.props.maxDate && isoString ? new Date(`${isoString.slice(0,10)}T12:00:00.000Z`) : null;
    ...
    if (selectedDate) {
      ...
      const minDateStr = minDate && minDate['toString'] ? minDate.toString():(typeof minDate==="string" ? minDate : null);
      const maxDateStr = maxDate && maxDate['toString'] ? maxDate.toString():(typeof maxDate==="string" ? maxDate : null);
      const todayStr = today.toString();
      if (minDate && Date.parse(minDateStr) >= Date.parse(todayStr)){
        displayDate = minDate;
      } else if (maxDate && Date.parse(maxDateStr) <= Date.parse(todayStr)){
        displayDate = maxDate;
      } else {
        displayDate = today;
      }
  ...
  handleInputChange() {
    ...
    const originalValue = ReactDOM.findDOMNode(this.refs.input).getAttribute('value');
  ...
  render() {
    ...
    return <InputGroup
      ...
      <Overlay
        ...
        delayHide={200}>
        <Popover id={`date-picker-popover-${this.props.fieldKey}`} className="date-picker-popover" title={calendarHeader}>
          <Calendar
